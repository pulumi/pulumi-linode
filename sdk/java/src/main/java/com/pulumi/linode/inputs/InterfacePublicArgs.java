// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.linode.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.linode.inputs.InterfacePublicIpv4Args;
import com.pulumi.linode.inputs.InterfacePublicIpv6Args;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class InterfacePublicArgs extends com.pulumi.resources.ResourceArgs {

    public static final InterfacePublicArgs Empty = new InterfacePublicArgs();

    /**
     * IPv4 addresses for this interface.
     * 
     */
    @Import(name="ipv4")
    private @Nullable Output<InterfacePublicIpv4Args> ipv4;

    /**
     * @return IPv4 addresses for this interface.
     * 
     */
    public Optional<Output<InterfacePublicIpv4Args>> ipv4() {
        return Optional.ofNullable(this.ipv4);
    }

    /**
     * IPv6 addresses for this interface.
     * 
     */
    @Import(name="ipv6")
    private @Nullable Output<InterfacePublicIpv6Args> ipv6;

    /**
     * @return IPv6 addresses for this interface.
     * 
     */
    public Optional<Output<InterfacePublicIpv6Args>> ipv6() {
        return Optional.ofNullable(this.ipv6);
    }

    private InterfacePublicArgs() {}

    private InterfacePublicArgs(InterfacePublicArgs $) {
        this.ipv4 = $.ipv4;
        this.ipv6 = $.ipv6;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(InterfacePublicArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private InterfacePublicArgs $;

        public Builder() {
            $ = new InterfacePublicArgs();
        }

        public Builder(InterfacePublicArgs defaults) {
            $ = new InterfacePublicArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param ipv4 IPv4 addresses for this interface.
         * 
         * @return builder
         * 
         */
        public Builder ipv4(@Nullable Output<InterfacePublicIpv4Args> ipv4) {
            $.ipv4 = ipv4;
            return this;
        }

        /**
         * @param ipv4 IPv4 addresses for this interface.
         * 
         * @return builder
         * 
         */
        public Builder ipv4(InterfacePublicIpv4Args ipv4) {
            return ipv4(Output.of(ipv4));
        }

        /**
         * @param ipv6 IPv6 addresses for this interface.
         * 
         * @return builder
         * 
         */
        public Builder ipv6(@Nullable Output<InterfacePublicIpv6Args> ipv6) {
            $.ipv6 = ipv6;
            return this;
        }

        /**
         * @param ipv6 IPv6 addresses for this interface.
         * 
         * @return builder
         * 
         */
        public Builder ipv6(InterfacePublicIpv6Args ipv6) {
            return ipv6(Output.of(ipv6));
        }

        public InterfacePublicArgs build() {
            return $;
        }
    }

}
