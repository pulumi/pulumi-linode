// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.linode.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class InstanceLinodeInterfaceVpcIpv4AddressArgs extends com.pulumi.resources.ResourceArgs {

    public static final InstanceLinodeInterfaceVpcIpv4AddressArgs Empty = new InstanceLinodeInterfaceVpcIpv4AddressArgs();

    /**
     * The SLAAC address chosen for this interface.
     * 
     */
    @Import(name="address")
    private @Nullable Output<String> address;

    /**
     * @return The SLAAC address chosen for this interface.
     * 
     */
    public Optional<Output<String>> address() {
        return Optional.ofNullable(this.address);
    }

    @Import(name="nat11Address")
    private @Nullable Output<String> nat11Address;

    public Optional<Output<String>> nat11Address() {
        return Optional.ofNullable(this.nat11Address);
    }

    /**
     * Whether the interface is the primary interface that should have the default route for this Linode. This field is only allowed for interfaces with the `public` or `vpc` purpose.
     * 
     * * `ipv4` - (Optional, Block) The IPv4 configuration of the VPC interface. Referenced with an index (e.g. `ipv4.0.vpc`). This field is currently only allowed for interfaces with the `vpc` purpose.
     * 
     * * `ipv6` - (Optional, Block) The IPv6 configuration of the VPC interface. Referenced with an index (e.g. `ipv6.0.is_public`). This field is currently only allowed for interfaces with the `vpc` purpose. NOTE: IPv6 VPCs may not yet be available to all users.
     * 
     */
    @Import(name="primary")
    private @Nullable Output<Boolean> primary;

    /**
     * @return Whether the interface is the primary interface that should have the default route for this Linode. This field is only allowed for interfaces with the `public` or `vpc` purpose.
     * 
     * * `ipv4` - (Optional, Block) The IPv4 configuration of the VPC interface. Referenced with an index (e.g. `ipv4.0.vpc`). This field is currently only allowed for interfaces with the `vpc` purpose.
     * 
     * * `ipv6` - (Optional, Block) The IPv6 configuration of the VPC interface. Referenced with an index (e.g. `ipv6.0.is_public`). This field is currently only allowed for interfaces with the `vpc` purpose. NOTE: IPv6 VPCs may not yet be available to all users.
     * 
     */
    public Optional<Output<Boolean>> primary() {
        return Optional.ofNullable(this.primary);
    }

    private InstanceLinodeInterfaceVpcIpv4AddressArgs() {}

    private InstanceLinodeInterfaceVpcIpv4AddressArgs(InstanceLinodeInterfaceVpcIpv4AddressArgs $) {
        this.address = $.address;
        this.nat11Address = $.nat11Address;
        this.primary = $.primary;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(InstanceLinodeInterfaceVpcIpv4AddressArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private InstanceLinodeInterfaceVpcIpv4AddressArgs $;

        public Builder() {
            $ = new InstanceLinodeInterfaceVpcIpv4AddressArgs();
        }

        public Builder(InstanceLinodeInterfaceVpcIpv4AddressArgs defaults) {
            $ = new InstanceLinodeInterfaceVpcIpv4AddressArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param address The SLAAC address chosen for this interface.
         * 
         * @return builder
         * 
         */
        public Builder address(@Nullable Output<String> address) {
            $.address = address;
            return this;
        }

        /**
         * @param address The SLAAC address chosen for this interface.
         * 
         * @return builder
         * 
         */
        public Builder address(String address) {
            return address(Output.of(address));
        }

        public Builder nat11Address(@Nullable Output<String> nat11Address) {
            $.nat11Address = nat11Address;
            return this;
        }

        public Builder nat11Address(String nat11Address) {
            return nat11Address(Output.of(nat11Address));
        }

        /**
         * @param primary Whether the interface is the primary interface that should have the default route for this Linode. This field is only allowed for interfaces with the `public` or `vpc` purpose.
         * 
         * * `ipv4` - (Optional, Block) The IPv4 configuration of the VPC interface. Referenced with an index (e.g. `ipv4.0.vpc`). This field is currently only allowed for interfaces with the `vpc` purpose.
         * 
         * * `ipv6` - (Optional, Block) The IPv6 configuration of the VPC interface. Referenced with an index (e.g. `ipv6.0.is_public`). This field is currently only allowed for interfaces with the `vpc` purpose. NOTE: IPv6 VPCs may not yet be available to all users.
         * 
         * @return builder
         * 
         */
        public Builder primary(@Nullable Output<Boolean> primary) {
            $.primary = primary;
            return this;
        }

        /**
         * @param primary Whether the interface is the primary interface that should have the default route for this Linode. This field is only allowed for interfaces with the `public` or `vpc` purpose.
         * 
         * * `ipv4` - (Optional, Block) The IPv4 configuration of the VPC interface. Referenced with an index (e.g. `ipv4.0.vpc`). This field is currently only allowed for interfaces with the `vpc` purpose.
         * 
         * * `ipv6` - (Optional, Block) The IPv6 configuration of the VPC interface. Referenced with an index (e.g. `ipv6.0.is_public`). This field is currently only allowed for interfaces with the `vpc` purpose. NOTE: IPv6 VPCs may not yet be available to all users.
         * 
         * @return builder
         * 
         */
        public Builder primary(Boolean primary) {
            return primary(Output.of(primary));
        }

        public InstanceLinodeInterfaceVpcIpv4AddressArgs build() {
            return $;
        }
    }

}
