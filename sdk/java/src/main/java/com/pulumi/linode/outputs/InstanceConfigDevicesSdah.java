// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.linode.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Integer;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class InstanceConfigDevicesSdah {
    /**
     * @return The Disk ID to map to this disk slot
     * 
     */
    private @Nullable Integer diskId;
    /**
     * @return The Block Storage volume ID to map to this disk slot
     * 
     */
    private @Nullable Integer volumeId;

    private InstanceConfigDevicesSdah() {}
    /**
     * @return The Disk ID to map to this disk slot
     * 
     */
    public Optional<Integer> diskId() {
        return Optional.ofNullable(this.diskId);
    }
    /**
     * @return The Block Storage volume ID to map to this disk slot
     * 
     */
    public Optional<Integer> volumeId() {
        return Optional.ofNullable(this.volumeId);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(InstanceConfigDevicesSdah defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Integer diskId;
        private @Nullable Integer volumeId;
        public Builder() {}
        public Builder(InstanceConfigDevicesSdah defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.diskId = defaults.diskId;
    	      this.volumeId = defaults.volumeId;
        }

        @CustomType.Setter
        public Builder diskId(@Nullable Integer diskId) {

            this.diskId = diskId;
            return this;
        }
        @CustomType.Setter
        public Builder volumeId(@Nullable Integer volumeId) {

            this.volumeId = volumeId;
            return this;
        }
        public InstanceConfigDevicesSdah build() {
            final var _resultValue = new InstanceConfigDevicesSdah();
            _resultValue.diskId = diskId;
            _resultValue.volumeId = volumeId;
            return _resultValue;
        }
    }
}
